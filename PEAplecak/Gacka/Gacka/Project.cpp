...
void CProject::algorithmName(char algorithm)
{
	...
	case '2':
		std::cout << "BRANCH & BOUND - WSZERZ" << std::endl;
		break;

	case '3':
		std::cout << "BRANCH & BOUND - BEST FIRST" << std::endl;
		break;

	case '4':
		std::cout << "PROGRAMOWANIE DYNAMICZNE" << std::endl;
		break;
...
void CProject::runAlgorithm(char algorithm)
{
	...
	case '2':
		handle->branchAndBoundBFS(true);
		break;

	case '3':
		handle->branchAndBoundBestFirst(true);
		break;
...
void CProject::debugMenu()
{
	...
	while (true)
	{
		...
		if (objectCreated)
		{
			...
			std::cout << "6. BRANCH & BOUND - WSZERZ" << std::endl;
			std::cout << "7. BRANCH & BOUND - BEST FIRST" << std::endl;
			std::cout << "8. ALGORYTM OPARTY O PROGRAMOWANIE DYNAMICZNE" << std::endl;
			std::cout << "9. ALGORYTM APROKSYMACYJNY FPTAS" << std::endl;
		}
...
void CProject::runTests(int numberOfElements, int* backpackSize, double error, std::ofstream &output)
{
	...
	for (int i = 0; i < 3; i++)
	{
		...
		output << "   BRANCH & BOUND - WSZERZ: " << handle->test('2', numberOfElements, backpackSize[i], error) << std::endl;
		output << "   BRANCH & BOUND - BEST FIRST: " << handle->test('3', numberOfElements, backpackSize[i], error) << std::endl;
		output << "   \"DYNAMICZNY\": " << handle->test('4', numberOfElements, backpackSize[i], error) << std::endl;
		output << "   APROKSYMACYJNY FPTAS: " << handle->test('5', numberOfElements, backpackSize[i], error) << std::endl;
		output << std::endl;
	}
}
